...
import 'package:bike_gps/constants/app_constants.dart';
...
class MapboxMapState extends State<MapboxMapWidget> {
  ...
  _drawRoute({
    ...
    Line touchAreaLine = await _mapController.addLine(
      LineOptions(
        ...
        lineColor: AppConstants.routeTouchAreaColor,
        lineOpacity: 0,
      ),
    );
    Line backgroundLine = await _mapController.addLine(
      LineOptions(
        ...
        lineColor: AppConstants.routeBorderColor,
        lineOpacity: 0.8,
    ...
    Line routeLine = await _mapController.addLine(
      LineOptions(
        geometry: lineCoordinateList,
        lineWidth: lineWidth,
        lineColor: isMainRoute
            ? AppConstants.primaryRouteColor
            : AppConstants.secondaryRouteColor,
      ),
    );
  ...
  _deactivateRouteLine(RouteLine routeLine) async {
    if (Platform.isLinux) {
      _mapController.updateLine(routeLine.route,
          LineOptions(lineColor: AppConstants.secondaryRouteColor));
      routeLine.route = Line(
          routeLine.route.id,
          routeLine.route.options.copyWith(
              LineOptions(lineColor: AppConstants.secondaryRouteColor)));
  ...
  _activateRouteLine(RouteLine routeLine) {
    if (Platform.isLinux) {
      _mapController.updateLine(routeLine.touchArea,
          LineOptions(lineColor: AppConstants.routeTouchAreaColor));
      _mapController.updateLine(routeLine.background,
          LineOptions(lineColor: AppConstants.routeBorderColor));
      _mapController.updateLine(routeLine.route,
          LineOptions(lineColor: AppConstants.primaryRouteColor));
    } else {
      for (Line line in routeLine.getLines()) {
        _mapController.removeLine(line);
      }
